...
- When using data-analysis:research command, parallel subagent calls can generate duplicate research documents - one from the subagent and one consolidated document from the main agent _(data-analysis, subagents, research, duplicates)_
...
- BigQuery connection test uses pattern 'bun run /path/to/bigquery-query.ts test' while ClickHouse uses 'bun run /path/to/clickhouse-query.ts test' _(bigquery, clickhouse, connection-testing, bun)_
